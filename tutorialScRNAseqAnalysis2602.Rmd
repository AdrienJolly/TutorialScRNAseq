...
# changing rownames to make it more "meaningful"
...
#save the dataset before lunch.
```{r}
saveRDS(sce,"sceMorning2502.rds")

```

#load the dataset
```{r}
sce <- readRDS("sceMorning2502.rds")
...
#reducedDim(sce, "PCA") <-  reducedDim(sce, "PCA")[1:6,]


clusters <- clusterRows(reducedDim(sce, "PCA")[,1:6], NNGraphParam(cluster.fun="louvain",cluster.args=list(resolution=0.2)))
...
#select DEG result for cluster 1
markersClust1 <- markers[[1]]

#order the list of Differentially expressed genes
orderedMarkersClust1 <- markersClust1[order(markersClust1$summary.logFC,decreasing=TRUE),]

rownames(orderedMarkersClust1)[1:30]

```
#### at this point  you extract top 30 marker genes per cluster and check for pathway enrichment using online tools such as EnrichR or Chatgpt. try out multiple cluster
...
sce$annotationInit <- NA

sce$annotationInit[which(sce$cluster%in%"1")] <- "Activated CD8 T cells"
sce$annotationInit[which(sce$cluster%in%"2")] <- "CD4 T cells_1"
sce$annotationInit[which(sce$cluster%in%"3")] <- "? CD4 T cells"
sce$annotationInit[which(sce$cluster%in%"4")] <- "CD14+ infl. monocytes"
sce$annotationInit[which(sce$cluster%in%"5")] <- "CD20+ B cells"
sce$annotationInit[which(sce$cluster%in%"6")] <- "Macrophages/ Myeloid cells"
sce$annotationInit[which(sce$cluster%in%"7")] <- "activated CD8 T cells"
sce$annotationInit[which(sce$cluster%in%"8")] <- "CD4 T cells_2"

sceTcells=sce[,c(wich(sce$cluster%in%"1"),wich(sce$cluster%in%"2"),wich(sce$cluster%in%"3"),wich(sce$cluster%in%"7"),wich(sce$cluster%in%"8"))]



plotUMAP(sce,colour_by="STAT4")

```
#plotting distribution of PCA components values per cell for each cluster
```{r}
pca <- reducedDim(sce, "PCA")[,1:6]
...
df <- data.frame(
    ...
    PC5 = pca[,5],
    PC6 = pca[,6],
    cluster = factor(sce$cluster)
)

# Function to generate one violin plot
make_plot <- function(pc) {
    ggplot(df, aes_string(x="cluster", y=pc)) +
        geom_violin() +
        theme_classic() +
        labs(title=pc, x="Cluster", y=pc)
}

plots <- lapply(paste0("PC",1:3), make_plot)
...
#finding top weighted genes per cell PC
```{r}

...
```{r}
sce <- read10xCounts("/Users/adrienjolly/Downloads/raw_feature_bc_matrix_1")
sce
```
...
#select specific column in colData
sce$Sample

#create a new annotation column for cells

sce$celltype <- NA

colData(sce)

#calling the count matrix
counts(sce)
...
```{r}
#save R object
saveRDS(sce,"sce_Morning2302.rds")

#load R object
sce <- readRDS("sce_Morning2302.rds")
```
...
sce <- runUMAP(sce, dimred="PCA", n_dimred=9)
```
...
# alternatively, you can choose to cluster based on a lower number of PCs, if you want to keep 9 PCs, just type 

clusters <- clusterRows(reducedDim(sce, "PCA")[,1:9], NNGraphParam(cluster.fun="louvain",cluster.args=list(resolution=0.2)))
sce$cluster <- factor(clusters)
...
#same with cluster 4
markersClust4=markers[[4]]

topClust4=markersClust4[order(markersClust4$summary.logFC,decreasing=TRUE),]


# and cluster 6

markersClust6=markers[[6]]



topClust6=markersClust6[order(markersClust6$summary.logFC,decreasing=TRUE),]
...
```{r}
#annotate cells based on marker genes
sce$Initannot=NA

sce$Initannot[which(sce$cluster%in%"1")] <- "CD4 T cells"
sce$Initannot[which(sce$cluster%in%"2")] <- "Activated T cells"
sce$Initannot[which(sce$cluster%in%"3")] <- "Monocytes, T cells?"
sce$Initannot[which(sce$cluster%in%"4")] <- "Inflammatory Mono."
sce$Initannot[which(sce$cluster%in%"5")] <- "B cells"
sce$Initannot[which(sce$cluster%in%"6")] <- "Monocytes/macrophages"
sce$Initannot[which(sce$cluster%in%"7")] <- "CD8 effector T cells"
sce$Initannot[which(sce$cluster%in%"8")] <- "Naive CD4 T cells"
sce$Initannot[which(sce$cluster%in%"9")] <- "Naive CD8 or CM T cells"
...
TcellsIndex=which(sce$cluster%in%"1"|sce$cluster%in%"2"|sce$cluster%in%"3"|sce$cluster%in%"7"|sce$cluster%in%"8")

sceTcells <- sce[,TcellsIndex]

saveRDS(markersT,"markersTcellDSub.rds")

plotUMAP(sce,colour_by="Initannot")
